...
4. [The Manifest (`Beru.toml`)](#4-the-manifest-berutoml)
5. [The Registry & Index](#5-the-registry--index)
6. [Recipes & Third-Party Code](#6-recipes--third-party-code)
7. [Architecture & Build Engine](#7-architecture--build-engine)
8. [Multiple Binaries (Loose Files)](#8-multiple-binaries-loose-files)
...
- **Declarative Manifests:** You only ever write a simple `Beru.toml` file. No complex profile scripts required.
- **PubGrub Resolution:** Unlike vcpkg or Conan which often fail with cryptic errors on version conflicts, Beru uses the battle-tested PubGrub algorithm (used by Dart's `pub` and Python's `uv`) to guarantee conflict-free version resolution and provide clear, human-readable explanations if dependencies are incompatible.
...
### Customizing Your Build (Legacy CMake Support)

Beru operates around `CMakeLists.txt`, but it is **not** a black box. If you run `beru init` in an existing project, or simply want to modify the files generated by `beru new`, you are completely free to do so!

Beru will **never** overwrite an existing `CMakeLists.txt`. You can edit this file to:
- Add custom compiler flags (`target_compile_options`)
- Define complex library targets and object libraries
- Integrate with non-Beru system dependencies

When you run `beru build`, Beru quietly resolves your dependencies in the background, generates a `beru-toolchain.cmake` file containing the paths to all cached libraries, and invokes your `CMakeLists.txt` using that toolchain. 

To link a Beru dependency in your custom `CMakeLists.txt`, simply use standard CMake commands:
```cmake
...
Beru enforces a standard project structure (often called a "convention over configuration" approach). This makes it easy for any C++ developer to jump into a Beru project and immediately understand where everything lives. 

When you run `beru new my_app`, Beru generates the following standard layout:

```text
my_app/
├── Beru.toml           # The package manifest
├── Beru.lock           # The dependency lockfile (generated after build)
├── CMakeLists.txt      # Legacy CMake interoperability file
...
### `Beru.toml` and `Beru.lock`
- **`Beru.toml`** is the manifest. It is written by you and contains your package metadata (name, version, C++ standard) and a declarative list of your dependencies. It is exactly analogous to `Cargo.toml` or `package.json`.
- **`Beru.lock`** is automatically generated by Beru. It contains the exact, cryptographically hashed versions of the dependencies that were resolved by the PubGrub algorithm. You should **never** edit this file manually. Commit it to version control if you are building an executable, but ignore it if you are building a library.

### The `src/` Directory
The `src/` directory is where all of your actual C++ source files (`.cpp`, `.cc`, `.cxx`) belong. 
- For an **executable** (`type = "executable"`), Beru expects to find a `main.cpp` inside `src/`. If you place multiple loose `.cpp` files in this directory, Beru can run them individually as separate scripts (see [Multiple Binaries](#8-multiple-binaries-loose-files)).
- For a **library** (`type = "library"`), this folder contains the private implementation details that are compiled into your static or shared library artifact.

### The `include/` Directory
If you are building a library or a header-only package, your **public-facing** headers belong in the `include/` directory. 
By convention, you should nest them inside a folder with your project's name (e.g., `include/my_lib/my_lib.hpp`). This prevents header filename collisions when other projects include your library (`#include <my_lib/my_lib.hpp>`). Beru automatically exports the `include/` directory to anyone who depends on your package.

### The `tests/` Directory
All of your unit tests and integration tests go here. While Beru's native testing orchestration is still expanding, placing your tests here keeps them separated from your application logic and allows standard CMake testing frameworks (like CTest) to easily discover them.

### `CMakeLists.txt`
Beru orchestrates CMake under the hood. For simple projects, you never need to look at or touch this file. However, if your project needs to do complex things (like linking to non-Beru system libraries like `pthread` or X11, adding custom compiler flags, or using Qt's `moc`), you can edit this file freely. Beru will never overwrite it once you start modifying it.

### The `build/` Directory (Not committed)
When you run `beru build`, Beru generates a `build/` directory. This is the output folder where CMake configures the project and where the final compiled binaries (`.o`, `.a`, `.so`, executables) are placed. This directory should be ignored by Git (Beru adds it to `.gitignore` automatically).

---

## 4. The Manifest (`Beru.toml`)
...
## 8. Multiple Binaries (Loose Files)
