...
fn auto_append_target(project_dir: &std::path::Path, target_name: &str, cpp_file: &std::path::Path) -> Result<()> {
    ...
    if !content.contains(&search1) && !content.contains(&search2) {
        ...
        println!("{} target '{}' to CMakeLists.txt", style("Auto-added").green().bold(), target_name);
    }
